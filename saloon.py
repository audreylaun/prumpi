...
def tallies(beer_count):
    tally_string = ""
    tally_string_unit = ""
    for i in range(beer_count):
        if (i+1) % 5 == 0:
            tally_string_unit = '卌'
            tally_string += tally_string_unit
            tally_string_unit = ''
        else:
            tally_string_unit += '|'
    tally_string += tally_string_unit
...
def run_saloon_game(num_coins, num_aces, num_spitballs, num_beers, bow, gem, backpack, hat, heels, labubu, happiness, HAPPINESS_MAX, volume_on, saloon_first_open, saloon_first_complete):
    ...
    beer_button_home = pygame.Rect(400,200,50,50)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if screen_mode == "home":
                    ...
                    elif sloth_rect.collidepoint(mouse_pos):
                        if not drink_active and num_coins >=5:
                            drink_active = True
        ...
        if drink_active and drink_pos:
            if drink_pos[0] < dino_pos[0] + 20 and not draining:
                # move cup across bar
                drink_pos[0] += drink_speed
            else:
                # start draining
                draining = True
                if liquid_height > 0:
                    liquid_height -= 1  # shrink liquid
                else:
                    # cup is empty -> reset
                    beer_count += 1
                    beer_text_number = beer_font.render(tallies(beer_count), True, (255, 255, 255))
        ...
        if screen_mode == "title":
            ...
            if bow:
                ...
                screen.blit(bow_img, ( dino_pos_alley[0]+160, dino_pos_alley[1] + 50))
            if gem:
                gem_img = pygame.transform.scale(gem_img, (10,10))
                # screen.blit(gem_img, (397,274))
                screen.blit(gem_img, (dino_pos_alley[0]+82, dino_pos_alley[1]+149))
            if hat:
            ...
            screen.blit(beer_text_number, (beer_button_home.x + 45, beer_button_home.y + 50))
            ...
            if first_load:
                ...
                screen.blit(sloth_text_1, (speech_bubble_rect.x+20, speech_bubble_rect.y+10))
                screen.blit(sloth_text_2, (speech_bubble_rect.x+20, speech_bubble_rect.y+50))
            ...
            screen.blit(quest1_text_2, (quest_1_text_pos[0], quest_1_text_pos[1]+30))
            screen.blit(quest_1_subtext, (quest_1_text_pos[0], quest_1_text_pos[1] + 55))
        ...
        if saloon_first_complete and quest1 and quest2 and quest3:
            current_screen = screen_mode
            screen_mode = "first complete"
            saloon_first_complete = False
        pygame.display.flip()
